...
function createNewDistancesMatrix(start: Square, heights: Matrix): Matrix {
...
for (let i = 0; i < heightMap.length; i++) {
  for (let j = 0; j < heightMap[0].length; j++) {
    if (heightMap[i][j] === 0) {
      const stepsToSquare = createNewDistancesMatrix([i, j], heightMap);
      const nextSquares: Array<Square> = [[i, j]];
      ...
      minSteps = Math.min(minSteps, stepsToSquare[end[0]][end[1]]);
    }
  }
}
